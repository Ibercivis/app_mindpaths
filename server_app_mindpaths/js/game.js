//Settings
var mindPathsBackEndUrl = "http://pybossa.socientize.eu/semantics-backend/";
...
function startOver() {
	...
	$("#wordImg" + steps).attr("src", "/static/app_mindpaths/img/endrow.png");
...
function setWordBarItem(index, checked, word) {
	...
	if (checked) {
		$(image).attr("src", "/static/app_mindpaths/img/activeRow.png");
		$(label).css({ color: '#2B3126' });
	}
	else {
		$(image).attr("src", "/static/app_mindpaths/img/empty_row.png");
...
function setWordBarEndItem(index) {
	var image = "#wordImg" + index;
	$(image).attr("src", "/static/app_mindpaths/img/endrow.png");
...
function toggleResetButton(state){
	...
	ChangeImage(document.getElementById('imgResetButton'), '/static/app_mindpaths/img/reset.png');
	resetState = state;
}
